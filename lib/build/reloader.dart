...
reloadProject(BuildProject prj){
  ...
  Map<String,dynamic> old = json.decode(file);
  ...
  prj.packs.forEach((pack) {
    ...
    Map newPack = newPrj["packs"][i];
    Map oldPack = old["packs"][i];
  ...
  if(old["name"] == newPack["name"]){
    ...
    if(newPack["files"] != null){
      ...
      for (var file in newPack["files"].keys) {
        if(old["files"][file] != null && old["files"][file].toString() == newPack["files"][file].toString()) 
